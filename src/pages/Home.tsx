...
type BodyweightView = 'week' | 'month' | 'allTime'
type PhaseMode = 'gain' | 'cut'

export default function Home() {
  const [selectedWorkout, setSelectedWorkout] =
    useState<WorkoutWithRelations | null>(null)
  const [bodyweightView, setBodyweightView] = useState<BodyweightView>('week')
  const [showSettings, setShowSettings] = useState(false)
  const [phaseMode, setPhaseMode] = useState<PhaseMode>(() => {
    const m = localStorage.getItem('mode')
    return m === 'cut' ? 'cut' : 'gain'
  })

  useEffect(() => {
    const onPhaseMode = (e: Event) => {
      const ce = e as CustomEvent<PhaseMode>
      if (ce.detail === 'gain' || ce.detail === 'cut') setPhaseMode(ce.detail)
    }
  ...
  const handleDelete = (_workoutId: string) => {

...
import type { BodyweightRow } from '@/types/workout'
...
function processBodyweightData(data: BodyweightRow[] | undefined): ChartRow[] {
  if (!data) return []
...
function processOverloadData(
  workoutData: ProgressWorkout[] | undefined
): ChartRow[] {
  if (!workoutData || workoutData.length < 2) return []

  const firstVolumeMap: Record<string, number> = {}
...
type ExerciseForProgress = {
  name: string
  workout: { date: string } | null
}

function processQualifiedExercises(
  data: ExerciseForProgress[] | undefined
): { name: string }[] {
  if (!data) return []
  const exerciseMap: Record<string, Set<string>> = {}
  data.forEach((ex) => {
...
export default function Progress() {
  ...
  const { data: rawBodyweight = [] } = useQuery({
    queryKey: ['bodyweight'],
  ...
  const overloadData = processOverloadData(
    workoutData as ProgressWorkout[] | undefined
  )
  const exercises = processQualifiedExercises(
    exercisesRaw as ExerciseForProgress[] | undefined
  )
  const exerciseData = processExerciseData(
    exerciseProgressRaw as ExerciseProgressEntry[] | undefined
  )

...
function ExerciseBlock({ exercise, onChange, onRemove }: ExerciseBlockProps) {
  ...
  const blockInvalidKeys = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(e.key) && Number.isNaN(Number(e.key)))
      e.preventDefault()
  }

  const blockInvalidKeysNoDecimal = (
...
export default function ActiveWorkout({
  ...
  const addExercise = (libraryExercise: ExerciseLibraryRow) => {
    const newExercise: DraftExercise = {
      ...
      muscles: (libraryExercise.exercise_library_muscles || []).map((m) => ({
  ...
  const updateExercise = (index: number, updated: DraftExercise) => {

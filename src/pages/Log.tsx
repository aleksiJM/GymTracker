...
export default function Log() {
  ...
  const [activeWorkout, setActiveWorkout] = useState<ActiveWorkoutDraft | null>(
    null
  )
  ...
  useEffect(() => {
    if (bodyweightData.length === 0) return
    queueMicrotask(() => {
      setBodyweightInput((prev) =>
        prev === '' ? String(bodyweightData[0].weight) : prev
      )
    })
  }, [bodyweightData])
  ...
  const repeatWorkout = (workout: WorkoutWithRelations) => {
    const exercises: DraftExercise[] = workout.exercises.map((ex) => ({
      name: ex.name,
      muscles: (ex.exercise_muscles || []).map((m) => ({

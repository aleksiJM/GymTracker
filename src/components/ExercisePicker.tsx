...
import type { ExerciseLibraryRow, MuscleDraft } from '@/types/workout'

type CreateExerciseFormProps = {
  name: string
  onCreated: (exercise: ExerciseLibraryRow) => void
}

function CreateExerciseForm({ name, onCreated }: CreateExerciseFormProps) {
  const [muscles, setMuscles] = useState<MuscleDraft[]>([])
  ...
  const removeMuscle = (muscle: MuscleDraft) => {
    setMuscles((prev) =>
      prev.filter(
        (m) =>
          !(
  ...
  const regions = selectedGroup
    ? MUSCLE_GROUPS[selectedGroup as keyof typeof MUSCLE_GROUPS]
    : []
  ...
  const handleCreate = async () => {
    ...
    onCreated({
      ...
      exercise_library_muscles:
        muscles as ExerciseLibraryRow['exercise_library_muscles'],
    })
    setSaving(false)
  }
      ...
      <div className='flex gap-2 mb-3 items-center'>
        ...
        >
          ...
          <SelectContent className='bg-card border-border'>
            {(Object.keys(MUSCLE_GROUPS) as (keyof typeof MUSCLE_GROUPS)[]).map(
              (mg) => (
        ...
        {regions.length > 0 && (
          <Select
            ...
            onValueChange={(val: string | null) => setSelectedRegion(val ?? '')}
          >
            <SelectTrigger className='flex-1 bg-card border-border text-foreground'>
...
type ExercisePickerProps = {
  isOpen: boolean
  onClose: () => void
  onSelect: (exercise: ExerciseLibraryRow) => void
}
...
export default function ExercisePicker({
  ...
  useEffect(() => {
    ...
    const fetchExercises = async () => {
      ...
      else setExercises((data ?? []) as ExerciseLibraryRow[])
      setLoading(false)
    }
    void fetchExercises()
  }, [isOpen])
  ...
  const handleCreated = (exercise: ExerciseLibraryRow) => {
    setExercises((prev) =>
  ...
  const handleSelect = (exercise: ExerciseLibraryRow) => {
    onSelect(exercise)
    setSearch('')
    onClose()
  }
            ...
            filtered.map((exercise) => (
              ...
              >
                <div className='flex-1 min-w-0 pr-2'>
                  ...
                  {exercise.exercise_library_muscles &&
                ...
                <div className='flex items-center gap-2 shrink-0'>
                  <button
                    ...
                    onClick={(e) => void handleDelete(e, exercise.id)}
                  >
                    &times;
                  </button>
                </div>
